    ...
    def format_properly(self, item):
        if item == None:
            ...
            if len(item) == self.num_joints:
                ...
                raise ValueError(
                    'List lenght does not match the number of joints! It should be long {}'.format(self.num_joints))
...
class Problem:
    ...
    def nlp_solver(self, initial_cond, final_term_cost, trajectory_target, my_constraints):
        ...
        if final_term_cost != None:
            J = J + final_term_cost(Xk_end[0:self.num_joints] - cs.substitute(self.traj, self.t, self.T),
                                    Xk_end[self.num_joints:] - cs.substitute(self.traj_dot, self.t, self.T),
                                    Uk)  # f_t_c(q, qd, u)    
    ...
    def add_constraints(self, g_, lbg_, ubg_, Xk_, Uk_, EEk_pos_, Q_ddot_, my_constraints_):
        for constraint in my_constraints_:
            l_bound, f_bound, u_bound = constraint(Xk_[0:self.num_joints], Xk_[self.num_joints:2 * self.num_joints],
                                                   Uk_, EEk_pos_, Q_ddot_)
    ...
    def derive_trajectory(self, traj, t):
        if isinstance(traj(t)[0], list):  # If user gave also traj_dot as input then
            ...
            traj_dot = [cs.jacobian(traj(t)[idx], t) for idx in
                        range(self.num_joints)]  # If user did not give traj_dot, then derive it from traj
